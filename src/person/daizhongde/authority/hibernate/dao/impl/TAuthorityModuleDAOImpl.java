 ...
 */
public class TAuthorityModuleDAOImpl extends SpringHibernateDaoSupport implements TAuthorityModuleDAO {
	private static final Logger log = LoggerFactory
			.getLogger(TAuthorityModuleDAOImpl.class);
...
//	}
	public List findChildrenRecursive( Integer parentId )
	{
		...
		String queryString = "select * " +
							  ...
							  "where FIND_IN_SET( N_Mid, f_getChildModuleList( :parentId ) ) " +
	...
	public List findChildrenRecursive_ExceptDev( Integer parentId )
	{
		...
		String queryString = "select * " +
							   "from "+ INIT.AUTH_schema +".t_authority_module " +
							  "where FIND_IN_SET( N_Mid, f_getChildModuleList( :parentId ) ) and N_Mid not in (98,9801,9802,9803,9804,9805) " +
							  "order by N_Mlevel,N_Morder";
		Map map = new HashMap();
		map.put( "parentId", parentId );
		return this.sqlQuerylistAllByMap( queryString, map, TAuthorityModule.class );
	...
	public List findChildrenRecursive( Integer parentId, Integer maxLevel )
	{
		...
		String queryString = "select * " +
							   "from "+ INIT.AUTH_schema +".t_authority_module " +
							  "where FIND_IN_SET( N_Mid, f_getChildModuleList( :parentId ) ) and N_MLEVEL <= "+maxLevel+" " +
							  "order by N_Mlevel,N_Morder";
		Map map = new HashMap();
		map.put( "parentId", parentId );
		return this.sqlQuerylistAllByMap( queryString, map, TAuthorityModule.class );
